...
([20171018_SoccertAI_筑波大蹴球部様へのご協力願い](20171018.pdf))
...
**詳しい説明は[ライブラリデータを新規作成](#id8)でします**

上記のPDFで支持されているように**15秒を1コマ**のプレーをいい，これを４つ合わせた1分のプレーをいくつか作っていただきます．
...
があるので，**15秒の中で必ず指定されたプレー位置で指定されたプレー種類があるようにしてください

またこれから*始点*と*終点*という用語が頻繁にでてきます．意味は：

* 始点：一番最初の選手配置

* 終点：一番最後の選手配置

始点と終点は既存のデータファイルがあるので，シミュレーションを作っていくときはこれらを自分で選択するところから始めます．

振り分けはデータ班グーグルドライブ上の役割分担.xlsxにあるので見ながらコマを作る必要があります．

プレー種類の定義は曖昧であるため，ある程度想像を働かしても作っていただいても結構です．

完成したらファイル名を**始点_終点_日付(201711XX)_人名_番号（自分がやった分数）.csv**に変更

以下役割分担.xlsxのスプレッドシート
![Wait for load](pics/Sketch3.png)

##### 完成した1分間シミュレーションの例：

![Wait for load](pics/gif1.gif)

### ログファイル作成
[絵コンテ・Log](#id12)を参考にして.logファイルを取得する．

ファイル名を**始点_終点_日付(201711XX)_人名_番号（自分がやった分数）_番号（コマに合わせて１～４のどれか）.log**に変更

##### 完成したlogの例：
![Wait for load](pics/Sketch6.png)


### 絵コンテ作成
[絵コンテ・Log](#id12)を参考にして.pngファイルを取得します．
...
テンプレの矢印と吹き出しを用いて，指定されたプレーの範囲でいいので説明を打ち込んでください．
                ...
                点線の矢印   →       ボールの動き
                
絵コンテが完成したら，4コマ分（1分）のスライドを保存し

ファイル名を**始点_終点_日付(201711XX)_人名_番号（自分がやった分数）.pptx**に変更

またそれぞれの絵コンテ（矢印と吹き出しも含めて）それぞれを画像(.jpeg)として保存する

        マウスをドラグして選手配置図，矢印，吹き出しを選択する．
        
        右クリックして、[図として保存] をクリックします．

        [ファイル名] ボックスにドキュメントの新しい名前を入力する．

        [保存] をクリックします．
        
ファイル名は**始点_終点_日付(201711XX)_人名_番号（自分がやった分数）_番号（コマに合わせて１～４のどれか）.jpg**にします．
...
完成し，グーグルドライブ上のフォルダにファイルをあげたらこうなります．
...
7.　ボールホルダーの選択
 ...
        **必ずボールホルダーを選択するようにしてください！！**

8.　キープレイヤーの選択
 
        選手を選択し、「キープレイヤー選択(複数名可)」ボタンをクリックします。
        **キープレイヤーはプレー種類で指定されたプレーが起きた前後に関わった敵と味方の選手を指定することが望ましいです！**
